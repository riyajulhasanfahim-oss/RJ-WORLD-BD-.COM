...
import { notifyVendorNewMessage } from '../../services/vendorNotificationService';
...
export default function ChatRoom() {
  ...
  // Fetch product info if productId is in search params
  useEffect(() => {
    if (!productId) return;
    let isMounted = true;
    const fetchProduct = async () => {
      try {
        const pSnap = await fetchProductById(productId);
        if (isMounted && pSnap) {
          setProductInfo(pSnap);
        }
      } catch (err) {
        console.warn("Could not fetch product info for chat", err);
      }
    };
    fetchProduct();
  ...
  const handleSend = async (e?: React.FormEvent, customImageUrl?: string, overrideText?: string) => {
    e?.preventDefault();
    if (uploadingImage) {
  ...
  return (
    <div className="h-[100dvh] max-h-[100dvh] bg-slate-100 flex flex-col font-sans overflow-hidden">
      {/* Show full website header on desktop only to maximize chat space on mobile */}
      ...
      <div className="flex-1 flex flex-col max-w-3xl mx-auto w-full bg-white sm:border-x sm:border-slate-200 overflow-hidden relative shadow-xs">
          ...
          <div className="flex items-center gap-2 min-w-0">
            ...
            >
              <div className="w-8 h-8 sm:w-9 sm:h-9 bg-sky-100 rounded-full flex items-center justify-center text-primary-main font-bold shrink-0 overflow-hidden border border-slate-200">
                {shopLogo ? (
                  <img src={shopLogo} alt={shopName} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                ) : (
                  <span className="text-xs sm:text-sm">{shopName[0]?.toUpperCase() || 'V'}</span>
                )}
              </div>
              <div className="min-w-0">
                <div className="flex items-center gap-1 min-w-0">
                  <h2 className="font-bold text-slate-900 text-xs sm:text-sm truncate group-hover:text-primary-main transition-colors">
                    {shopName}
                  </h2>
                  {isStorePlanVerified(vendorInfo) && (
                    <VerifiedBadge title="অনুমোদিত ভেরিফাইড স্টোর" size="xs" />
                  )}
                </div>
        ...
        {/* Compact Product Inquiry Preview (if opened from product page) */}
        {productInfo && (
          <div className="bg-sky-50/90 border-b border-sky-100 px-3 py-1.5 sm:px-4 sm:py-2 flex items-center justify-between gap-2 shrink-0">
            <div className="flex items-center gap-2 min-w-0">
              <img 
                src={productInfo.images?.[0] || productInfo.thumbnail || productInfo.image || 'https://via.placeholder.com/50'} 
                alt={productInfo.title || productInfo.name} 
                className="w-7 h-7 sm:w-8 sm:h-8 rounded-md object-cover border border-sky-200 shrink-0 bg-white"
                referrerPolicy="no-referrer"
              />
              <div className="min-w-0">
                <p className="text-[11px] sm:text-xs font-semibold text-slate-800 truncate">
                  {productInfo.title || productInfo.name}
                </p>
                <p className="text-[10px] sm:text-[11px] font-bold text-primary-main">
                  ৳{productInfo.price || productInfo.salePrice || 0}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-1.5 shrink-0">
              <button 
                onClick={() => handleSend(undefined, undefined, `I want to inquire about: ${productInfo.title || productInfo.name} (৳${productInfo.price})`)}
                className="text-[10px] sm:text-xs bg-primary-main text-white font-bold px-2 py-1 rounded-md hover:bg-sky-600 transition-colors shadow-2xs"
              >
                Send Product Link
              </button>
              <button 
          ...
          {loading ? (
            ...
            messages.map(msg => {
              ...
              return (
                ...
                >
                  {/* Quick delete icon on hover for user's own message */}
                  {isMine && (
                    <button
                      type="button"
                  ...
                  {/* Quick delete icon on hover for vendor's message */}
                  {!isMine && (
                    <button
                      type="button"
                      onClick={() => setSelectedMessageForAction(msg)}
