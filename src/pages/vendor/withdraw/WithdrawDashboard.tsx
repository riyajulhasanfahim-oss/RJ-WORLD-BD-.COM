...
export default function WithdrawDashboard() {
  ...
  // Fetch Vendor Wallet & Withdrawal History
  const fetchData = async () => {
    if (!user) return;
    try {
      setHistoryLoading(true);
      // 1. Fetch Wallet from RTDB
      let walletData = await rtdbGet<any>(`vendor_wallet/${user.uid}`);
      if (walletData) {
      ...
      // 2. Fetch Withdraws from RTDB
      const [wList, reqList] = await Promise.all([
        rtdbList<any>('withdrawals', (w) => w.vendorId === user.uid || (w.userId === user.uid && w.accountType === 'Vendor')).catch(() => []),
        rtdbList<any>('withdraw_requests', (w) => w.vendorId === user.uid).catch(() => [])
      ]);

      const map = new Map<string, any>();
      [...wList, ...reqList].forEach(item => {
      ...
      console.error("Error fetching vendor withdraw data:", error);
    } finally {
  ...
  useEffect(() => {
    ...
    const unsub = rtdbSubscribe<any>('withdrawals', (snap) => {
      ...
      Object.keys(snap).forEach(key => {
        const item = snap[key];
        if (item && (item.vendorId === user.uid || (item.userId === user.uid && item.accountType === 'Vendor'))) {
      ...
      if (vendorWithdrawals.length > 0) {
        vendorWithdrawals.sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));
        setHistory(vendorWithdrawals);
      }
    });
  ...
  const withdrawableBalance = Number(wallet?.balance) || 0;
  const pendingAmount = useMemo(() => {
    return history
      .filter(h => h.status === 'Pending')
      .reduce((sum, h) => sum + (h.amount || 0), 0);
  }, [history]);
  const displayTotal = withdrawableBalance + (wallet?.pendingBalance || pendingAmount);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const requestData: any = {
        userId: user.uid,
        vendorId: user.uid,
        userName: wallet?.storeName || user.displayName || 'Vendor',
      ...
      // 2. Also save to 'withdraw_requests' with same key for backward compatibility
      await rtdbUpdate(`withdraw_requests/${withdrawKey}`, { id: withdrawKey, ...requestData }).catch(() => {});

      // 3. Deduct Vendor Wallet and update pendingBalance in RTDB
      await rtdbUpdate(`vendor_wallet/${user.uid}`, {
        balance: Math.max(0, withdrawableBalance - numAmount),
        pendingBalance: (wallet?.pendingBalance || 0) + numAmount,
        updatedAt: Date.now()
      });
      ...
      await rtdbPush('wallet_transactions', {
        ...
        description: `Vendor withdraw request (${method} - ${cleanNumber})`,
        createdAt: Date.now()
      }).catch(() => {});
      ...
      await rtdbPush('vendor_notifications', {
        userId: 'admin',
        title: 'নতুন ভেন্ডর উইথড্র রিকোয়েস্ট',
        message: `${wallet?.storeName || 'ভেন্ডর'} ৳${numAmount} উত্তোলনের রিকোয়েস্ট পাঠিয়েছে (${method} - ${cleanNumber})`,
        read: false,
        type: 'withdraw',
        withdrawId: withdrawKey,
        timestamp: Date.now()
            ...
            <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 rounded-2xl p-4 sm:p-5 text-white shadow-md relative overflow-hidden">
              ...
              <div className="relative z-10 mt-3 pt-2.5 border-t border-white/10 flex flex-wrap items-center justify-between gap-2 text-xs">
                <div className="flex items-center gap-1.5 text-slate-300">
                  ...
                  <span>প্রসেসিংয়ে আছে: <strong className="text-amber-400">৳{(wallet?.pendingBalance || pendingAmount).toFixed(2)}</strong></span>
                </div>
          ...
          <div className="lg:col-span-5 space-y-4">
            ...
            <div className="bg-sky-50/70 rounded-2xl p-3.5 border border-sky-100 flex items-center justify-between">
              <div>
                ...
                <p className="text-[11px] text-slate-500">আমাদের হেল্পলাইনে সরাসরি যোগাযোগ করুন</p>
              </div>
              <button
