...
import { rtdbPush, rtdbList, rtdbGet, rtdbUpdate, rtdbSubscribe } from '../../lib/rtdb';
...
export default function WithdrawPage() {
  ...
  // Load recent withdrawal requests from both RTDB and Firestore
  const fetchWithdrawalsHistory = async () => {
    if (!user) return;
    try {
      setHistoryLoading(true);
      const [rtdbListRes, firestoreSnap] = await Promise.all([
      ...
      const rtdbItems = rtdbListRes.map(item => ({ id: item.id, ...item.data }));
      const firestoreItems = firestoreSnap.docs.map(d => ({ id: d.id, ...d.data() }));

      const map = new Map<string, any>();
      [...rtdbItems, ...firestoreItems].forEach(item => {
        if (item.id && !map.has(item.id)) {
          map.set(item.id, {
            id: item.id,
            ...item,
            method: item.method || item.paymentMethod || 'bKash',
            accountNumber: item.accountNumber || item.mobileNumber || item.paymentNumber || '',
            createdAt: item.createdAt || item.date || Date.now()
          });
        }
      });

      const list = Array.from(map.values()) as WithdrawalRecord[];
  ...
  useEffect(() => {
    ...
    const unsub = rtdbSubscribe<any>('withdrawals', (snap) => {
      if (!snap) return;
      const userWithdrawals: WithdrawalRecord[] = [];
      ...
      if (userWithdrawals.length > 0) {
        userWithdrawals.sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));
        setHistory(userWithdrawals);
      }
    });
  ...
  const withdrawableBalance = isBonusLocked ? baseBalance : baseBalance + lockedBonus;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!method) {
      toast.error('অনুগ্রহ করে একটি উইথড্র মেথড বেছে নিন');
      return;
    }
    ...
    if (numAmount > withdrawableBalance) {
      if (numAmount <= displayBalance && isBonusLocked) {
        toast.error(`৳${lockedBonus} বোনাস লক করা আছে। ১০টি সফল সেল সম্পন্ন হলে এটি আনলক হবে।`);
      } else {
    ...
    try {
      ...
      // 1. Push to RTDB 'withdrawals' (Admin Panel listens to this)
      const withdrawKey = await rtdbPush('withdrawals', newRecord);

      // 2. Also save to Firestore for backward compatibility
      await addDoc(collection(db, 'withdrawals'), {
        id: withdrawKey,
        ...newRecord
      }).catch(() => {});

      // 3. Deduct User Wallet and move to held balance in RTDB
      try {
        const curUser = await rtdbGet<any>(`users/${user.uid}`) || {};
        const curWallet = typeof curUser.wallet === 'number' ? curUser.wallet : (typeof curUser.balance === 'number' ? curUser.balance : (userData.wallet || 0));
        const newWallet = Math.max(0, curWallet - numAmount);
        const curHeld = Number(curUser.heldBalance) || 0;
        await rtdbUpdate(`users/${user.uid}`, {
          wallet: newWallet,
          balance: newWallet,
          walletBalance: newWallet,
          heldBalance: curHeld + numAmount,
          updatedAt: Date.now()
        });

        // Also sync Firestore users doc if exists
        try {
        ...
        console.warn('Error adjusting user balance in RTDB:', balErr);
      }

      // 4. Add Transaction Record in RTDB
      await rtdbPush('wallet_transactions', {
        ...
        description: `উইথড্র রিকোয়েস্ট (${method} - ${cleanNumber})`,
        createdAt: Date.now()
      }).catch(() => {});
      ...
      await rtdbPush('vendor_notifications', {
        userId: 'admin',
        title: 'নতুন ইউজার উইথড্র রিকোয়েস্ট',
        message: `${userData.name || 'ইউজার'} ৳${numAmount} উত্তোলনের রিকোয়েস্ট পাঠিয়েছে (${method} - ${cleanNumber})`,
        read: false,
        type: 'withdraw',
        withdrawId: withdrawKey,
        timestamp: Date.now()
        ...
        <div className="max-w-4xl mx-auto w-full">
          ...
          <div className="flex items-center justify-between mb-3 sm:mb-5">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[11px] sm:text-xs text-slate-500">
                  bKash, Nagad ও Rocket এর মাধ্যমে দ্রুত টাকা উত্তোলন
                </p>
              </div>
            </div>
            ...
            <button 
              ...
              className="hidden sm:flex items-center gap-1.5 text-xs font-bold text-primary-main bg-sky-50 px-3 py-1.5 rounded-xl hover:bg-sky-100 transition-colors"
            >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-6 items-start">
            ...
            {/* Left Column (Main Withdrawal Form) */}
            <div className="lg:col-span-7 space-y-4">
              ...
              <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 rounded-2xl p-4 sm:p-5 text-white shadow-md relative overflow-hidden">
                ...
                {/* Sub info row: Pending / Locked Bonus */}
                <div className="relative z-10 mt-3 pt-2.5 border-t border-white/10 flex flex-wrap items-center justify-between gap-2 text-xs">
                  <div className="flex items-center gap-1.5 text-slate-300">
                    ...
                    <span>প্রসেসিংয়ে আছে: <strong className="text-amber-400">৳{pendingAmount.toFixed(2)}</strong></span>
                  </div>

                  <div className="flex items-center gap-1.5 text-emerald-400 font-semibold text-[11px]">
                    <Zap className="w-3.5 h-3.5" />
                    <span>উইথড্র ফি: ৳০ (ফ্রি)</span>
                  </div>
                </div>

                {isBonusLocked && (
                  <div className="relative z-10 mt-2.5 bg-amber-500/20 border border-amber-400/30 rounded-xl p-2.5 flex items-start gap-2 text-xs text-amber-200">
                    <Lock className="w-3.5 h-3.5 text-amber-400 shrink-0 mt-0.5" />
                    <span className="leading-snug text-[11px]">
                      ৳{lockedBonus} বোনাস লক রয়েছে ({eligibleSales}/10 সেলস সম্পন্ন)।
                    </span>
                  </div>
                )}
              </div>

              {/* Withdrawal Method Selection (bKash, Nagad, Rocket) */}
            ...
            <div className="lg:col-span-5 space-y-4">
              ...
              <div className="bg-white rounded-2xl p-4 shadow-2xs border border-slate-200/80 space-y-3">
                ...
                <div className="space-y-2 text-xs text-slate-600">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      <strong className="text-slate-800">অটোমেটেড ও সুরক্ষিত:</strong> কোনো থার্ড-পার্টি চার্জ নেই, ১০০% নিরাপদ ডিজিটাল ট্রান্সফার।
                    </div>
                  </div>
              ...
              <div className="bg-white rounded-2xl p-4 shadow-2xs border border-slate-200/80">
                <div className="flex items-center justify-between mb-3 pb-2 border-b border-slate-100">
                  ...
                  <button 
                    onClick={fetchWithdrawalsHistory} 
                    className="text-[11px] font-semibold text-primary-main hover:underline"
                  >
                    রিফ্রেশ
                  </button>
                </div>
              ...
              <div className="bg-sky-50/70 rounded-2xl p-3.5 border border-sky-100 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[11px] text-slate-500">আমাদের কাস্টমার কেয়ার প্রতিনিধিকে জানান</p>
                </div>
                <button
