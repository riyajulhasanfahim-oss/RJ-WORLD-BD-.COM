...
export interface RocketPaymentModalProps {
  isOpen: boolean;
  onClose: () => void;
  onBack: () => void;
  amount: number;
...
export default function RocketPaymentModal({
  isOpen,
  onClose,
  onBack,
  amount,
    ...
    >
      ...
      >
          ...
          <div className="col-span-4 border border-gray-200 rounded-xl sm:rounded-2xl p-2 sm:p-3 flex items-center justify-center bg-white shadow-2xs">
            <div className="flex items-center justify-center h-9 sm:h-10 w-full">
              <svg viewBox="0 0 110 38" className="h-7 sm:h-8 w-auto" fill="none" xmlns="http://www.w3.org/2000/svg">
                {/* Dutch-Bangla Rocket Icon */}
                <path d="M12 28L24 6L32 20L12 28Z" fill="#8c3494"/>
                <path d="M24 6L38 12L32 20L24 6Z" fill="#aa42b4"/>
                <path d="M32 20L42 30L22 25L32 20Z" fill="#6d1e75"/>
                
                {/* Rocket label */}
                <text x="44" y="11" fontFamily="system-ui, -apple-system, sans-serif" fontWeight="900" fontSize="8" fill="#8c3494">ROCKET</text>
                {/* Bengali text "রকেট" */}
                <text x="42" y="24" fontFamily="system-ui, -apple-system, sans-serif" fontWeight="900" fontSize="16" fill="#8c3494">রকেট</text>
                <text x="43" y="32" fontFamily="system-ui, -apple-system, sans-serif" fontWeight="500" fontSize="5" fill="#666666">একটি ডাচ-বাংলা ব্যাংক মোবাইল ব্যাংকিং</text>
              </svg>
            </div>
          </div>
        </div>

        {/* Purple Rocket Instruction Box */}
        <div className="bg-[#81207e] rounded-2xl p-3.5 sm:p-5 text-white shadow-md mb-3 sm:mb-4">
          {/* Box Header */}
          ...
          <div className="mb-3.5">
            ...
            />
            {errorMessage && (
              <div className="mt-2.5 p-2 bg-purple-950/90 border border-yellow-300/60 rounded-xl text-yellow-100 text-xs font-bold text-center shadow-inner">
                {errorMessage}
              </div>
            )}
          </div>
          ...
          <div className="text-[11px] sm:text-[13px] font-medium leading-relaxed space-y-2 select-none">
            {/* 1 */}
            <div className="border-b border-purple-400/40 pb-2">
              <span className="font-bold mr-1.5">•</span>
              *322# ডায়াল করে আপনার ROCKET মোবাইল মেনুতে যান অথবা ROCKET অ্যাপে যান।
            </div>
            ...
            {/* 3: ROCKET Number with Copy Button */}
            <div className="border-b border-purple-400/40 pb-2 flex flex-wrap items-center">
              <span className="font-bold mr-1.5">•</span>
              <span>প্রাপক নম্বর হিসেবে এই নম্বরটি লিখুনঃ&nbsp;</span>
              <span className="text-[#ffd200] font-extrabold tracking-wider">{rocketNumber}</span>
              <button
                type="button"
            ...
            <div className="border-b border-purple-400/40 pb-2">
              <span className="font-bold mr-1.5">•</span>
              নিশ্চিত করতে এখন আপনার ROCKET মোবাইল মেনু পিন লিখুন।
            </div>

            {/* 6 */}
            <div className="border-b border-purple-400/40 pb-2">
              <span className="font-bold mr-1.5">•</span>
              সবকিছু ঠিক থাকলে, আপনি ROCKET থেকে একটি নিশ্চিতকরণ বার্তা পাবেন।
            </div>
        ...
        <button
          ...
          className="w-full bg-[#81207e] hover:bg-[#6f186c] active:bg-[#5e135b] disabled:opacity-50 text-white font-black py-3.5 sm:py-4 px-6 rounded-xl sm:rounded-2xl text-base sm:text-lg shadow-md transition-all active:scale-[0.98] flex items-center justify-center tracking-wider select-none touch-manipulation cursor-pointer min-h-[50px] sm:min-h-[54px]"
        >
          {isSubmitting ? (
