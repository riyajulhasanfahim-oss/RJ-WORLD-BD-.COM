...
if (content.includes('profile?.verificationBadge')) {
    ...
    // Add BadgeCheck import if missing
...
// 2. Add Search bar above product grid
const targetGrid = `<div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">`;
const searchBar = `
                    {/* Search and Filters */}
...
if (content.includes(targetGrid) && !content.includes('Search products')) {
    content = content.replace(targetGrid, searchBar);
}
